...
echo "Configuring Jenkins..."
    ...
    workspaceDir=$(sed -n '/^  <workspaceDir>${JENKINS_HOME}\/workspace\/${ITEM_FULL_NAME}<\/workspaceDir>/=' "/var/lib/jenkins/config.xml")
    if [ ! -z "$workspaceDir" ]; then
        sed -i "${workspaceDir}s/.*/  <workspaceDir>\/app\/shopgular-backend<\/workspaceDir>/" "/var/lib/jenkins/config.xml"
    fi
...
echo "Starting backend container..."
    cd /app/shopgular-backend && docker compose up -d
...
echo "Starting frontend container..."
    cd /app/shopgular-frontend && docker compose up -d
